...
function Header() {
  ...
  const menuHandler = () => {
    show.menu
      ...
      : setShow({ ...show, menu: true });
  };
  // show/hide submenu
  const subMenuHandler = () => {
    show.subMenu
      ? setShow({ ...show, subMenu: false })
      : setShow({ ...show, subMenu: true });
  };
  // hide menu
  show.menu &&
    document.addEventListener("click", (e) => {
      if (
        !e.target.parentElement.classList.contains("header-nav") &&
  ...
  return (
    <header
      id="header"
      className={`header ${show.change ? "header-secondary" : ""} ${
      ...
      <nav className="header-nav container container-lg">
        ...
        <ul className={`menu ${show.menu ? "show" : ""}`}>
          <li className="menu-item">
            <Link to="/" className="menu-item-link menu-link">
              home
            </Link>
          </li>

          <li className="menu-item">
            <Link to="/about" className="menu-item-link menu-link">
              about
            </Link>
          </li>

          <li className="menu-item">
            <Link to="/services" className="menu-item-link menu-link">
              services
            </Link>
          </li>

          <li className="menu-item work">
            <Link to="/work" className="menu-item-link menu-link">
              work
            </Link>
            ...
            <button
              className={`submenu-btn ${
                show.subMenu ? "icon-up" : "icon-down"
              }`}
            ...
            <ul className={`submenu ${show.subMenu ? "show-submenu" : ""}`}>
              <li className="submenu-item">
                <Link to="/project01" className="submenu-link menu-link">
                  florida heath facility
                </Link>
              </li>

              <li className="submenu-item">
                <Link to="/project02" className="submenu-link menu-link">
                  maine modernity
                </Link>
              </li>

              <li className="submenu-item">
                <Link to="/project03" className="submenu-link menu-link">
                  exclusive urban living
                </Link>
              </li>

              <li className="submenu-item">
                <Link to="/project04" className="submenu-link menu-link">
                  blue concrete house
                </Link>
              </li>
            </ul>
          ...
          <li className="menu-item">
            <Link to="/get-quote" className="menu-item-link menu-link">
              <span className="btn btn-primary">get quote</span>
            </Link>
          </li>
        </ul>
        ...
        <button
          className={
            show.menu ? "menu-hamburger icon-close" : "menu-hamburger icon-menu"
          }
          onClick={menuHandler}
        ></button>
      </nav>
